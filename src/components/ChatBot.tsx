...
export default function ChatBot({ plan }: ChatBotProps) {
  ...
  return (
    ...
    >
      {/* ── Header ──────────────────────────────── */}
      <header
        className={`shrink-0 px-4 py-3 sm:py-4 flex items-center justify-between relative overflow-hidden ${isPremium
            ? "bg-mrt-950"
            : "bg-mrt-950"
          } text-white`}
      >
        {isPremium && (
          <div className="absolute inset-0 bg-gradient-to-r from-gold-900/20 via-transparent to-gold-900/20 pointer-events-none" />
        )}
        <div className="flex items-center gap-3 relative z-10">
          <div
            className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center overflow-hidden shrink-0 transition-transform hover:scale-105 duration-300 ${isPremium
                ? "bg-gradient-to-br from-mrt-800 to-mrt-900 border border-gold-500/30 shadow-[0_0_10px_rgba(229,173,66,0.2)]"
                : "bg-mrt-800 border border-mrt-700"
              }`}
          >
            <Image
              src="/logo.png"
              ...
              width={24}
              height={20}
              className={`invert ${isPremium ? "drop-shadow-[0_0_5px_rgba(229,173,66,0.5)]" : "opacity-90"}`}
            />
          </div>
          <div className="min-w-0">
            <h1 className="font-bold text-sm sm:text-base leading-tight flex items-center gap-2 truncate">
              {isPremium ? (
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-gold-300 to-gold-500">Asesoria MRT</span>
              ) : (
                "Asesoria MRT"
              )}
              {isPremium && (
                <span className="text-[9px] bg-gold-500/20 border border-gold-500/30 text-gold-400 px-2 py-0.5 rounded-md font-bold tracking-widest uppercase flex items-center gap-1 shrink-0">
                  <Crown size={10} /> Pro
                </span>
              )}
            </h1>
            <p className="text-[10px] sm:text-[11px] text-mrt-400 leading-tight font-medium tracking-wide truncate mt-0.5">
      ...
      {/* ── Messages ────────────────────────────── */}
      <div className={`flex-1 overflow-y-auto px-4 sm:px-5 py-5 sm:py-6 space-y-4 sm:space-y-5 custom-scrollbar relative ${isPremium ? "bg-mrt-50/30" : "bg-mrt-50/50"}`}>
        {isPremium && (
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 bg-gold-500/5 rounded-full blur-[80px] pointer-events-none"></div>
        )}
          ...
          >
            {msg.sender === "bot" && (
              <div className={`w-7 h-7 sm:w-8 sm:h-8 rounded-xl flex items-center justify-center shrink-0 mr-2 sm:mr-3 mt-1 shadow-sm border ${isPremium ? "bg-gradient-to-br from-gold-100 to-white border-gold-200" : "bg-white border-mrt-200"
                ...
                <Image src="/logo.png" alt="" width={16} height={14} className={isPremium ? "opacity-90" : "opacity-60"} />
              </div>
            )}
            <div className={`max-w-[85%] sm:max-w-[80%] flex flex-col ${msg.sender === "user" ? "items-end" : "items-start"}`}>
              <div
                className={`px-4 sm:px-5 py-3 text-[13px] sm:text-sm leading-relaxed whitespace-pre-wrap shadow-sm ${msg.sender === "user"
                    ? `rounded-2xl rounded-tr-sm ${isPremium
                      ? "bg-gradient-to-br from-mrt-900 to-mrt-800 text-white border border-mrt-700 shadow-[0_4px_15px_-4px_rgba(0,0,0,0.3)]"
                      : "bg-mrt-900 text-white"
                    }`
                    : `rounded-2xl rounded-tl-sm bg-white text-mrt-800 border ${isPremium ? "border-gold-200/60 shadow-[0_2px_10px_-2px_rgba(229,173,66,0.1)]" : "border-mrt-200"
                    }`
                  }`}
              >
                {msg.text}
